            ...
            try {
                ...
                if (action === 'fix') {
                    ...
                    for (const issue of issues.filter(i => i.type === 'ready_but_archived')) {
                        const pattern = new RegExp(`### ${issue.taskId}: ${issue.title?.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')} ⬜ READY`);
                        if (pattern.test(updatedQueue)) {
                            updatedQueue = updatedQueue.replace(pattern, `### ${issue.taskId}: ${issue.title} ✅ DONE`);
                            fixedCount++;
                        }
                    }
                    ...
                    updatedQueue = updatedQueue.replace(/\| ⬜ READY \| \d+ \|/, `| ⬜ READY | ${newReadyCount} |`);
                    updatedQueue = updatedQueue.replace(/\| ✅ DONE \| \d+ \|/, `| ✅ DONE | ${newDoneCount} |`);
