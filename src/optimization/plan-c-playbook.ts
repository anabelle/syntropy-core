...
/**
 * Configuration options for the optimization workflow
 */
export interface OptimizationConfig {
  /** Target memory limit in MiB (default: 1024MiB for 1GiB) */
  targetMemoryMB: number;

  /** Current memory limit in MiB */
  currentMemoryMB: number;

  /** Container name to optimize */
  containerName: string;

  /** Validation duration in hours (default: 24h for production) */
  validationDurationHours: number;

  /** Whether to enable swap sustainability testing */
  testSwapSustainability: boolean;

  /** Custom thresholds for alerts */
  thresholds?: {
    /** Memory usage percentage threshold (default: 85%) */
    memoryPercent: number;

    /** Load per core threshold (default: 1.5) */
    loadPerCore: number;

    /** Swap usage percentage threshold (default: 50%) */
    swapPercent: number;
  };
}

/**
 * Optimization metrics captured during analysis phase
 */
export interface OptimizationMetrics {
  /** Baseline memory before optimization (MiB) */
  baselineMemoryMB: number;

  /** Memory after optimization (MiB) */
  optimizedMemoryMB: number;

  /** Memory reduction percentage */
  reductionPercent: number;

  /** Load average before/after (1min/5min/15min) */
  loadAvg: {
  ...
  containerHealth: 'healthy' | 'unhealthy' | 'starting' | 'no-healthcheck';
...
/**
 * Optimization technique with execution details
 */
export interface OptimizationTechnique {
  ...
  category: 'configuration' | 'resource-limits' | 'data-management' | 'process-tuning';
...
/**
 * Validation result from production testing
 */
export interface ValidationResult {
  ...
  /** Container uptime during validation */
  ...
  /** Any issues detected during validation */
  issues: string[];

  /** Whether swap sustainability was tested */
  swapSustainabilityTested: boolean;

  /** Swap stability rating (1-5, 5 = most stable) */
  swapStabilityRating?: number;
}
 ...
 */
export const DEFAULT_TECHNIQUES: OptimizationTechnique[] = [
  {
 ...
 */
export const BITCOIN_TECHNIQUES: OptimizationTechnique[] = [
  {
    id: 'bitcoin-dbcache',
...
export class PlanCOptimizer {
  ...
  constructor(config: OptimizationConfig, techniques: OptimizationTechnique[] = DEFAULT_TECHNIQUES) {
    this.config = {
   ...
   */
  async tune(): Promise<OptimizationTechnique[]> {
    console.log(`[Plan C] Phase 2: TUNE - Applying ${this.techniques.length} techniques`);

    const applied: OptimizationTechnique[] = [];
   ...
   */
  static getTechniquesForContainer(containerType: 'bitcoin' | 'database' | 'cache' | 'general'): OptimizationTechnique[] {
    switch (containerType) {
      case 'bitcoin':
        return [...BITCOIN_TECHNIQUES, ...DEFAULT_TECHNIQUES.filter(t => t.category !== 'data-management')];
      case 'database':
        return DEFAULT_TECHNIQUES.filter(t =>
          t.id === 'docker-resource-limits' ||
   ...
   */
  private getContainerMemory(containerName: string): number {
   ...
   */
  private getContainerCPU(containerName: string): number {
    // In production, this would query docker stats
    // Simulated for the playbook
    if (containerName.includes('bitcoin')) {
      return 2.0;
   ...
   */
  private getContainerHealth(containerName: string): OptimizationMetrics['containerHealth'] {
   ...
   */
  private getServiceHealth(): Record<string, boolean> {
   ...
   */
  private async executeTechnique(technique: OptimizationTechnique): Promise<void> {
   ...
   */
  private calculateSwapStability(): number {
...
export async function optimizeContainer(
  containerName: string,
  containerType: 'bitcoin' | 'database' | 'cache' | 'general' = 'general',
 ...
 */
export function validateSwapSustainability(
  swapUsagePercent: number,
  loadPerCore: number,
  uptimeDays: number
): { sustainable: boolean; rating: number; explanation: string } {
  let rating = 0;
  const checks: string[] = [];
 ...
 */
export function getOptimizationPlaybookDocumentation(): string {
  return `
# Plan C Optimization Playbook
