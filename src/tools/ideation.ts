 ...
 */
function parseAllSeeds(garden: string): SeedInfo[] {
 ...
 */
function findSimilarSeeds(newTitle: string, newContent: string, existingSeeds: SeedInfo[]): Array<{ seed: SeedInfo; similarity: number }> {
  const newKeywords = extractKeywords(`${newTitle} ${newContent}`);
  const matches: Array<{ seed: SeedInfo; similarity: number }> = [];
 ...
 `;
        ...
        if (action === 'plant') {
          ...
          if (similarSeeds.length > 0) {
            const suggestions = similarSeeds.slice(0, 3).map(({ seed, similarity }) =>
              `- "${seed.title}" (${Math.round(similarity * 100)}% similar, ${seed.waterings} waterings)`
            ).join('\n');
...
- **Log**:
 `;
        ...
        if (action === 'harvest') {
          if (!seedTitle) return { error: "'seedTitle' is required" };
...
1. ...
 `;
        ...
        if (action === 'merge') {
          ...
          updatedPrimaryContent = updatedPrimaryContent.replace(
            /- \*\*Waterings\*\*: \d+/,
            `- **Waterings**: ${totalWaterings}`
          );
